...
#convert to bam and sort each
samtools view -b ${OUT_PREF}_paired_aligned.sam > ${OUT_PREF}_paired_aligned.bam 2>&1 | tee -a ${OUT_PREF}_all_log.txt
samtools sort ${OUT_PREF}_paired_aligned.bam -o ${OUT_PREF}_paired_aligned_sorted.bam 2>&1 | tee -a ${OUT_PREF}_all_log.txt
samtools view -b ${OUT_PREF}_single_aligned.sam > ${OUT_PREF}_single_aligned.bam 2>&1 | tee -a ${OUT_PREF}_all_log.txt
samtools sort ${OUT_PREF}_single_aligned.bam -o ${OUT_PREF}_single_aligned_sorted.bam 2>&1 | tee -a ${OUT_PREF}_all_log.txt

#merge bam files
samtools merge ${OUT_PREF}_aligned_sorted.bam ${OUT_PREF}_paired_aligned_sorted.bam ${OUT_PREF}_single_aligned_sorted.bam 2>&1 | tee -a ${OUT_PREF}_all_log.txt
